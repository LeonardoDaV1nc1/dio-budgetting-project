...
- **Como Utilizar na Interface:**
  - Realize o cadastro e login de um usuário para obter o token JWT.
  - Na página do Swagger, utilize o botão **Authorize** para inserir o token no formato `Bearer SEU_TOKEN`.
  - Explore e teste os endpoints protegidos de forma interativa e visual.

## 🧪 Como Testar o Fluxo Principal

Você pode interagir com os endpoints REST da aplicação simulando os comandos do assistente (garantindo o envio do token JWT autenticado):

- **Exemplo de transação em tempo real:**
  - *"Gastei 50 reais em alimentação hoje"* (Aciona a ferramenta `persist-transaction` vinculada ao seu usuário)
- **Exemplo de transação retroativa:**
  - *"Gastei 120 reais em mercado no dia 10 do mês passado"* (Aciona a ferramenta `persist-past-transaction`)
- **Exemplo de remoção:**
  - *"Apague minha última transação"* (Aciona a ferramenta `remover-last-transaction` afetando apenas o seu histórico)
...
- **Integração Prática de IA com Backend:** Aprofundei o uso de métodos Java seguros (`@Tool`) guiados por um modelo de linguagem integrado a uma arquitetura limpa e validada.